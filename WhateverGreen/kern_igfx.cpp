...
void IGFX::applyWestmerePatches(KernelPatcher &patcher) {
	...
	if (kernelVersion == KernelVersion::SnowLeopard) {
		const uint8_t findStride[] = { 0x0F, 0x85, 0x75, 0xFE, 0xFF, 0xFF, 0xE9, 0x6B, 0xFE, 0xFF, 0xFF };
		const uint8_t replaceStride[] = { 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0xE9, 0x6B, 0xFE, 0xFF, 0xFF };
	...
	if (framebufferPatchFlags.bitsWestmere.SingleLink && framebufferWestmerePatches.SingleLink) {
		...
		if (kernelVersion == KernelVersion::SnowLeopard) {
			const uint8_t findSingleWidth2Ml[] = { 0xBE, 0x00, 0x00, 0x00, 0x09 };
			const uint8_t replaceSingleWidth2Ml[] = { 0xBE, 0x00, 0x00, 0x00, 0x08 };
			...
			const uint8_t findSingleWidth2Ml[] = { 0xB8, 0x00, 0x00, 0x00, 0x09 };
			const uint8_t replaceSingleWidth2Ml[] = { 0xB8, 0x00, 0x00, 0x00, 0x08 };
			KernelPatcher::LookupPatch singleWidth2MlPatch { currentFramebuffer, findSingleWidth2Ml, replaceSingleWidth2Ml, sizeof(findSingleWidth2Ml), 1 };
			patcher.applyLookupPatch(&singleWidth2MlPatch);
	...
	if (kernelVersion == KernelVersion::SnowLeopard) {
		const uint8_t findLinkWidthMl[] = { 0x49, 0x8B, 0x85, 0x98, 0x06, 0x00, 0x00, 0x0F, 0xB6, 0x40, 0x18, 0xC1, 0xE0, 0x13, 0x41, 0x0B, 0x44, 0x24, 0x6C, 0x41, 0x89, 0x44, 0x24, 0x6C, 0x49, 0x8B, 0x95, 0x98, 0x00, 0x00, 0x00, 0x89, 0x82, 0x0C, 0x00, 0x0F, 0x00, 0x49, 0x8B, 0x85, 0x98, 0x06, 0x00, 0x00, 0x0F, 0xB6, 0x40, 0x18, 0xC1, 0xE0, 0x13, 0x41, 0x0B, 0x44, 0x24, 0x68 }
		const uint8_t replaceLinkWidthMl[] = { 0x41, 0x8B, 0x44, 0x24, 0x6C, 0x25, 0xFF, 0xFF, 0xC7, 0xFF, 0x0D, linkWidthPtr[3], linkWidthPtr[2], linkWidthPtr[1], linkWidthPtr[0], 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x49, 0x8B, 0x95, 0x98, 0x00, 0x00, 0x00, 0x89, 0x82, 0x0C, 0x00, 0x0F, 0x00, 0x41, 0x8B, 0x44, 0x24, 0x68, 0x25, 0xFF, 0xFF, 0xC7, 0xFF, 0x0D, 0x00, 0x00, 0x08, 0x00, 0x90, 0x90, 0x90, 0x90 };
		...
		DBGLOG("igfx", "applyWestmerePatches applied link width patch for Snow Leopard");
	} else if (kernelVersion == KernelVersion::Lion) {
		const uint8_t findLinkWidthMl[] = { 0x49, 0x8B, 0x86, 0x98, 0x06, 0x00, 0x00, 0x0F, 0xB6, 0x40, 0x18, 0xC1, 0xE0, 0x13, 0x0B, 0x43, 0x6C, 0x89, 0x43, 0x6C, 0x49, 0x8B, 0x8E, 0x98, 0x00, 0x00, 0x00, 0x89, 0x81, 0x0C, 0x00, 0x0F, 0x00, 0x49, 0x8B, 0x86, 0x98, 0x06, 0x00, 0x00, 0x0F, 0xB6, 0x40, 0x18, 0xC1, 0xE0, 0x13, 0x0B, 0x43, 0x68 }
		const uint8_t replaceLinkWidthMl[] = { 0x8B, 0x43, 0x6C, 0x25, 0xFF, 0xFF, 0xC7, 0xFF, 0x0D, linkWidthPtr[3], linkWidthPtr[2], linkWidthPtr[1], linkWidthPtr[0], 0x90, 0x90, 0x90, 0x90, 0x89, 0x43, 0x6C, 0x49, 0x8B, 0x8E, 0x98, 0x00, 0x00, 0x00, 0x89, 0x81, 0x0C, 0x00, 0x0F, 0x00, 0x8B, 0x43, 0x68, 0x25, 0xFF, 0xFF, 0xC7, 0xFF, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x90, 0x90, 0x90, 0x90 }
		KernelPatcher::LookupPatch linkWidthPatchMl { currentFramebuffer, findLinkWidthMl, replaceLinkWidthMl, sizeof(findLinkWidthMl), 1 };
		patcher.applyLookupPatch(&linkWidthPatchMl);
		DBGLOG("igfx", "applyWestmerePatches applied link width patch for Lion");
	} else if (kernelVersion >= KernelVersion::MountainLion && kernelVersion <= KernelVersion::ElCapitan) {
