...
static IOReturn wrapFBClientDoAttribute(void *fbclient, uint32_t attribute, unsigned long* unk1, unsigned long unk2, unsigned long* unk3, unsigned long* unk4, void* externalMethodArguments) {
	...
	if (attribute == 0x923) {
		SYSLOG("igfx", "FBClientDoAttribute -> disabling AGDC!");
...
void IGFX::processKernel(KernelPatcher &patcher, DeviceInfo *info) {
	...
	if (info->videoBuiltin) {
		...
		switchOffFramebuffer = !blackScreenPatch && !applyFramebufferPatch && !dumpFramebufferToDisk && !dumpPlatformTable && !hdmiAutopatch && cflBacklightPatch == CoffeeBacklightPatch::Off && !maxLinkRatePatch && !hdmiP0P1P2Patch && !supportLSPCON && !forceCompleteModeset.enable && FRAMEBUFFER_DEBUG == 0;
